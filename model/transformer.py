...
class DecoderAdaLN(nn.Module):
    ...
    def __init__(self, hidden_dim):
        ...
        self.norm_proj = nn.Linear(hidden_dim * 2, hidden_dim)
...
class Attention(nn.Module):
    def __init__(self, num_embedding, num_heads, is_casual=False):
        super().__init__()
...
class AttentionBlock(nn.Module):
    def __init__(self, num_embedding, num_heads, norm=nn.LayerNorm, is_casual=False):
        super().__init__()
        self.ln_1 = norm(num_embedding)
        self.attn = Attention(num_embedding, num_heads, is_casual=is_casual)
        self.ln_2 = norm(num_embedding)
        self.mlp = MLP(num_embedding)
